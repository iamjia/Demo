...
@implementation YYImage {
    ...
    NSMutableDictionary<NSNumber *, UIImage *> *_preloadedFrames;
...
- (instancetype)initWithData:(NSData *)data maxPixelSize:(NSUInteger)maxPixelSize
{
...
- (instancetype)initWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    @autoreleasepool {
        _preloadedFrames = NSMutableDictionary.dictionary;
        YYImageDecoder *decoder = [YYImageDecoder decoderWithData:data scale:scale];
        decoder.maxPixelSize = _maxPixelSize;
    ...
    if (_preloadAllAnimatedImageFrames != preloadAllAnimatedImageFrames) {
        if (preloadAllAnimatedImageFrames && _decoder.frameCount > 0) {
            for (NSUInteger i = 0, max = _decoder.frameCount; i < max; i++) {
                [self animatedImageFrameAtIndex:i];
            }
        } else {
...
- (UIImage *)animatedImageFrameAtIndex:(NSUInteger)index {
    ...
    dispatch_semaphore_wait(_preloadedLock, DISPATCH_TIME_FOREVER);
    [_preloadedFrames setObject:image ?: (UIImage *)NSNull.null forKey:@(index)];
    dispatch_semaphore_signal(_preloadedLock);

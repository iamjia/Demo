...
- (CGImageRef)_newUnblendedImageAtIndex:(NSUInteger)index
    ...
    if (_source) {
        BOOL fetchThumbnail = 0 != _maxPixelSize;
        NSDictionary *options = nil;
        if (fetchThumbnail) {
            NSMutableDictionary *opt = NSMutableDictionary.dictionary;
            opt[(NSString *)kCGImageSourceShouldAllowFloat] = @YES;
            opt[(NSString *)kCGImageSourceCreateThumbnailFromImageAlways] = @YES;
            opt[(NSString *)kCGImageSourceThumbnailMaxPixelSize] = @(_maxPixelSize);
            options = opt;
        } else {
            options = @{(id)kCGImageSourceShouldCache:@YES};
        }

        CGImageRef imageRef = fetchThumbnail ? CGImageSourceCreateThumbnailAtIndex(_source, index, (CFDictionaryRef)options) : CGImageSourceCreateImageAtIndex(_source, index, (CFDictionaryRef)options);
        if (NULL != imageRef && extendToCanvas) {
            size_t width = CGImageGetWidth(imageRef);
            size_t height = CGImageGetHeight(imageRef);
            if (fetchThumbnail || (width == _width && height == _height)) {
                ...
                if (context) {
                    // !!!: _height 小于 height 时， _height - height 不会返回负数
                    CGFloat y = 0.0f;
                    if (_height < height) {
                        y = (height - _height) * -1.0f;
                    } else {
                        y = _height - height;
                    }
                    
                    CGContextDrawImage(context, CGRectMake(0, y, width, height), imageRef);
